MugenController = MeteorisController.extend({
    ...
    getCriteria: function() {
        ...
        return {
            $or: [
                {title: {$regex: search, $options: 'i'}},{content: {$regex: search, $options: 'i'}},
            ]
        };
    },
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Mugen.find(this.getCriteria(), sort);
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Mugen.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Mugen");
    ...
    update: function(t) {
        ...
        if (this._post) {
            ...
            Mugen.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Mugen");
            });
            Router.go('templateView', {_id: _id});
        }
    ...
    remove: function(_id) {
        Mugen.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Mugen");
        });
    },
    _loadModel: function(_id) {
        return Mugen.findOne(_id);
    },
});